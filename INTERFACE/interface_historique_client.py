...
HISTORIQUE = pygame.Rect(1,320,150,30)
TEXT_HISTORIQUE = FONT.render('Historique  ',1, WHITE )
TEXT_HISTORIQUE_CLICK = FONT.render('''Historique de ce client : Malte ............. ''',1, BLACK )

VOYAGE_A_VENIR = pygame.Rect(1,120,150,30)
TEXT_VOYAGE_CLICK = FONT.render('''Voyage à venir pour ce client :Malte ............. ''',1, BLACK )
...
TEXT_VOYAGE = FONT.render('Voyage : ',1,WHITE)
...
def draw_window_9():
    WINDOW.fill(WHITE)
    pygame.draw.rect(WINDOW,BLACK,VOYAGE_A_VENIR)
    pygame.draw.rect(WINDOW,BLACK,HISTORIQUE)
    ...
    while run : 
        ...
        for event in pygame.event.get() :
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  # vérifie si l'utilisateur a cliqué sur l'écran
                ...
                if HISTORIQUE.collidepoint(mouse_pos):
                    display_text2 = True 
        ...
        if display_text:  # si la variable est True, alors on affiche le texte
            WINDOW.blit(TEXT_VOYAGE_CLICK, (200, 150))
        if display_text2:
            WINDOW.blit(TEXT_HISTORIQUE_CLICK,(200,400))
